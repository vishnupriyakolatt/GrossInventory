...
function Product() {
  ...
  const fetchBooks = async () => {
    try {
      const response = await axios.get('http://localhost:9003/api/inventory/getallproducts');
...
<>
  ...
  <div className="flex flex-wrap h-20px justify-center gap-4">
  {products.map((product) => (
   <div key={product.id} className="relative mt-4 flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
      ...
      <div className="mt-4 px-5 pb-5">
        ...
        <h6 className="text-xl tracking-tight bg-#c2e9fb text-slate-900">{product.category}</h6>
